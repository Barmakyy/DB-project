...
CREATE TABLE students (
    ...
    CONSTRAINT chk_enrollment_before_graduation CHECK (graduation_date IS NULL OR enrollment_date <= graduation_date)
...
CREATE TABLE courses (
    ...
    CONSTRAINT fk_course_department FOREIGN KEY (department_id) 
        REFERENCES departments(department_id) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT chk_credits_range CHECK (credits BETWEEN 0.5 AND 6.0),
...
CREATE TABLE semesters (
    ...
    CONSTRAINT chk_registration_dates CHECK (registration_start < registration_end AND registration_end <= start_date)
);
...
CREATE TABLE enrollments (
    ...
    grade_letter ENUM('A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'F', 'W', 'I', 'P', 'NP'),
    ...
    CONSTRAINT fk_enrollment_section FOREIGN KEY (section_id) 
        REFERENCES course_sections(section_id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE payments (
    ...
    payment_method ENUM('Credit Card', 'Debit Card', 'Bank Transfer', 'Check', 'Cash', 'Scholarship', 'Financial Aid') NOT NULL,
    payment_purpose ENUM('Tuition', 'Housing', 'Meal Plan', 'Library Fine', 'Other') NOT NULL,
...
CREATE TABLE library_books (
    ...
    CONSTRAINT chk_copies_available CHECK (available_copies BETWEEN 0 AND total_copies),
...
CREATE TABLE book_loans (
    ...
    CONSTRAINT fk_loan_book FOREIGN KEY (book_id) 
        REFERENCES library_books(book_id) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE student_achievements (
    ...
    student_id INT NOT NULL,
    achievement_type ENUM('Academic', 'Athletic', 'Artistic', 'Community Service', 'Leadership', 'Other') NOT NULL,
...
CREATE TABLE disciplinary_actions (
    ...
    action_type ENUM('Warning', 'Probation', 'Suspension', 'Expulsion', 'Other') NOT NULL,
    description TEXT NOT NULL,
    date_issued DATE NOT NULL,
    issuing_authority VARCHAR(100) NOT NULL,
    end_date DATE,
    is_active TINYINT(1) DEFAULT 1,
    CONSTRAINT fk_action_student FOREIGN KEY (student_id) 
        REFERENCES students(student_id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT chk_end_after_issue CHECK (end_date IS NULL OR end_date >= date_issued)
);
...
CREATE INDEX idx_book_loan_student ON book_loans(student_id);
CREATE INDEX idx_book_loan_status ON book_loans(status);
